...
max_it = 200    # Maximum number of iterations/generations
...
# Set time limit to ensure termination within one minute
...
def swap_mutation(tour, mutation_rate):
    """Mutate a tour by swapping cities with probability mutation_rate."""
...
def get_best_individual(population, fitness_values):
    ...
    return population[best_idx], calculate_tour_length(population[best_idx])

def genetic_algorithm():
    """Execute the Genetic Algorithm with time safety."""
    ...
    for generation in range(max_it):
        ...
        current_best, current_length = get_best_individual(population, fitness_values)
        ...
        # If we didn't complete the next generation due to time constraints,
        # make sure we have at least a valid population
        if len(next_generation) < pop_size:
            # Fill remaining spots with copies of the best individuals
            while len(next_generation) < pop_size:
                best_idx = sorted_indices[len(next_generation) % len(sorted_indices)]
