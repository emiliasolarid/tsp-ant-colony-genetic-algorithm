...
max_it = 120  # Number of iterations
...
# Function to generate a random tour (fallback solution)
def generate_random_tour():
    """Generate a random tour for fallback."""
...
def select_next_city(current_city, unvisited, pheromone, dist_matrix, alpha, beta):
    ...
    for city in unvisited:
        ...
        # To avoid division by zero, add a small constant to distance
...
def construct_tour(pheromone, dist_matrix, alpha, beta):
    # Check time before starting tour construction
    ...
    while unvisited:
        # Periodically check if time is running out during tour construction
        if len(unvisited) % 20 == 0 and time_is_running_out():
            # Complete tour greedily if running out of time
            while unvisited:
...
def update_pheromone(pheromone, all_tours, tour_lengths, rho, Q):
    # Check time before starting pheromone update
    if time_is_running_out():
        return pheromone
...
# Main ACO algorithm (with time safety)
def ant_colony_optimization(dist_matrix, num_cities, max_it, num_ants, alpha, beta, rho, Q):
    # Create fallback valid tour
    ...
    # Adjust parameters for larger problems
    if num_cities > 100:
    ...
    for iteration in range(max_it):
        ...
        for ant in range(num_ants):
            ...
            tour = construct_tour(pheromone, dist_matrix, alpha, beta)
        ...
        # Update pheromone levels if time permits and we have tours
        if not time_is_running_out() and all_tours:
            pheromone = update_pheromone(pheromone, all_tours, tour_lengths, rho, Q)
    
    return best_tour, best_tour_length

# Run the ACO algorithm with time safety
try:
    tour, tour_length = ant_colony_optimization(dist_matrix, num_cities, max_it, num_ants, alpha, beta, rho, Q)
