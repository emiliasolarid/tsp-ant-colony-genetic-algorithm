...
# Scale parameters dynamically based on problem size for better performance
pop_size = min(100, max(50, num_cities))  # Population size scaled to problem
max_it = min(200, max(50, num_cities * 2))  # Maximum iterations scaled to problem

# Core parameters
tournament_size = 3  # Size of tournament for selection
elite_size = 2       # Number of elite individuals to keep
mutation_rate = 0.2  # Base mutation rate

# Enhanced parameters
crossover_rate = 0.9          # Probability of performing crossover
local_search_iterations = 5   # Limit local search iterations for speed
adaptive_phase = True         # Enable/disable adaptive phase selection

# Auto-adjust parameters for larger problems
if num_cities > 100:
    pop_size = 40
    max_it = 120
...
def time_is_running_out():
    ...
    return elapsed > time_limit * 0.8  # Return True if 80% of time has passed
...
def create_initial_population(pop_size):
    """Create an initial population with some heuristic initialization."""
    population = []
    
    # 70% random tours
    for _ in range(int(pop_size * 0.7)):
        population.append(generate_random_tour())
    
    # 30% nearest neighbor tours with different starting points
...
def fitness(tour):
    """Calculate fitness of a tour (inverse of tour length)."""
    tour_length = calculate_tour_length(tour)
    # Return inverse to convert minimization problem to maximization
    return 1.0 / (tour_length + 1.0)

def evaluate_population(population):
    """Evaluate fitness of each individual in the population."""
    return [fitness(tour) for tour in population]
...
def order_crossover(parent1, parent2):
    """Implement Order Crossover (OX) for permutation-based problems."""
    # Choose two crossover points randomly
    cx_points = sorted(random.sample(range(num_cities), 2))
    ...
    for i in range(num_cities):
        # Get city from parent2 (wrapping around if needed)
        city = parent2[(cx_points[1] + i) % num_cities]
        
        # If city is not already in offspring, add it
        if city not in offspring:
            # Find the next unfilled position
            while pointer < num_cities and offspring[pointer] != -1:
...
def swap_mutation(tour, mutation_rate):
    """Mutate a tour by swapping cities."""
    mutated_tour = tour.copy()
    
    for i in range(num_cities):
        # With probability mutation_rate, swap this city with another random city
...
def inversion_mutation(tour, mutation_rate):
    ...
    if random.random() < mutation_rate:
        ...
        pos1, pos2 = sorted(random.sample(range(num_cities), 2))

        # Reverse the segment between pos1 and pos2
        mutated_tour[pos1:pos2+1] = reversed(mutated_tour[pos1:pos2+1])
...
def two_opt_local_search(tour, max_iterations):
    ...
    # For large problems, only consider a subset of possible moves
    if num_cities > 100:
        # Sample potential edges to check
        edge_pairs = []
        for i in range(num_cities - 3):
            # Only check a subset of pairs
            j_values = random.sample(range(i + 2, num_cities - 1), min(5, num_cities - i - 3))
            edge_pairs.extend([(i, j) for j in j_values])
    else:
        # For smaller problems, check all possibilities or a subset
        edge_pairs = [(i, j) for i in range(num_cities - 3) for j in range(i + 2, min(i + 20, num_cities - 1))]
    
    # Shuffle to randomize the search
    random.shuffle(edge_pairs)
    
    while improved and iteration < max_iterations and not time_is_running_out():
        improved = False
        iteration += 1

        for i, j in edge_pairs:
            if j >= len(best_tour):  # Safety check
                continue

            # Calculate the change in tour length if we reverse the segment
            current_length = (dist_matrix[best_tour[i]][best_tour[i+1]] + 
                             dist_matrix[best_tour[j]][best_tour[(j+1) % num_cities]])
            new_length = (dist_matrix[best_tour[i]][best_tour[j]] + 
                         dist_matrix[best_tour[i+1]][best_tour[(j+1) % num_cities]])
...
def genetic_algorithm():
    """Execute the Enhanced Genetic Algorithm with time limit guarantee."""
    # Always have a fallback solution ready in case of timeout
    fallback_tour = generate_random_tour()
    fallback_length = calculate_tour_length(fallback_tour)
    ...
    for generation in range(max_it):
        ...
        current_length = calculate_tour_length(current_best)
        ...
        sorted_indices = sorted(range(len(population)), key=lambda i: fitness_values[i], reverse=True)
        for i in range(elite_size):
            next_generation.append(population[sorted_indices[i]])
        
        # Fill the rest of the population with offspring
        while len(next_generation) < pop_size and not time_is_running_out():
            # Select parents
            ...
            # Increase mutation rate if search stagnates
            current_mutation_rate = mutation_rate
            if stagnation_counter > 10:
                current_mutation_rate = min(0.5, mutation_rate * (1 + stagnation_counter/20))
            
            # Choose mutation operator based on random choice
            if random.random() < 0.5:
                offspring = swap_mutation(offspring, current_mutation_rate)
            else:
                offspring = inversion_mutation(offspring, current_mutation_rate)
            
            # Apply local search occasionally
            if random.random() < 0.1 and not time_is_running_out():
                offspring = two_opt_local_search(offspring, local_search_iterations)
        ...
        # Early termination if we're not improving and running out of time
        if stagnation_counter > max_it // 4 and time_is_running_out():
            break
    
    # If we still have time, apply final improvement to best tour
    if not time_is_running_out():
        best_tour = two_opt_local_search(best_tour, local_search_iterations * 2)
        best_length = calculate_tour_length(best_tour)
    
    return best_tour, best_length

# Run the Genetic Algorithm with time safety
try:
